...
export default class Dropzone extends Vue {
  ...
  onFilelistChanged(filelist: File[]) {
    if (
  ...
  mounted() {
    if (this.$slots && this.$slots.default && this.$slots.default[0].context)
      (this.$slots.default[0].context.$el as HTMLElement)
        .querySelectorAll('[dropzone-clickable]')
        .forEach((el) => {
  ...
  dragover(event: Event) {
    ...
    document.querySelector('[dropzone-style]')?.classList.add('dragover');
  ...
  dragleave(event: Event) {
    ...
    document.querySelector('[dropzone-style]')?.classList.remove('dragover');
